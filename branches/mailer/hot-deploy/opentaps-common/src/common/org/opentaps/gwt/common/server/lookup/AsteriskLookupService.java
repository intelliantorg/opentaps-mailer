    ...
    public String findCallInParty() throws InfrastructureException, EntityNotFoundException, RepositoryException, QueryException, GenericEntityException, SQLException {
        ...
        if (UtilProperties.getPropertyValue("asterisk.properties", "asterisk.enabled", "N").equals("Y")) {
            if (getProvider().getUser() != null) {
            	Debug.logInfo("########## getProvider().getUser() = "+getProvider().getUser().getUserId(), MODULE);
                DomainsLoader domainLoader = new DomainsLoader(getProvider().getInfrastructure(), getProvider().getUser());
                PartyDomainInterface partyDomain = domainLoader.loadDomainsDirectory().getPartyDomain();
                Debug.logInfo("########## partyDomain = "+partyDomain, MODULE);
                
                PartyRepositoryInterface repository = partyDomain.getPartyRepository();
                Debug.logInfo("########## repository = "+repository, MODULE);
                ...
                Debug.logInfo("########## extension = "+extension, MODULE);
                if (!extension.equals("")) {
                    ...
                    if (!callInNumber.equals("")) {
                        ...
                        if (parties.size() > 0) {
                            ...
                            Debug.logInfo("########## Find Call in Party, PartyId=" + party.getPartyId() + " ,url=" + link + ", userLogin >> " + getProvider().getUser().getUserId() + ", extn >> " + extension, MODULE);
                            return link;
                        }
                        Debug.logInfo("########## PhoneNumber:" + callInNumber + " cannot found any match party tried with >> " + getProvider().getUser().getUserId() + ", extn >> " + extension, MODULE);

                    } else {
                        Debug.logInfo("########## Cannot find any call in number tried with >> " + getProvider().getUser().getUserId(), MODULE);
                    }
                } else {
                    Debug.logError("########## Current UserLogin do not have any extension set tried with >> " + getProvider().getUser().getUserId(), MODULE);
                }
