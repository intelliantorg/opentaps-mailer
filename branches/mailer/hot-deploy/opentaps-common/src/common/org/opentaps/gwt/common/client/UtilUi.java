...
public abstract class UtilUi {
    ...
    public static final Integer INPUT_LENGTH = 200;
    ...
    /**
     * Popups an error dialog with the given error message, with a callback to execute once the dialog is closed.
     * @param error a <code>String</code> value
    ...
    public static void errorMessage(String error, MessageBox.PromptCallback cb) {
        logError(error, MODULE, "errorMessage");
    ...
    private static String toString(Iterable<Object> list) {
        StringBuilder sb = new StringBuilder("[");
        for (Object o : list) {
    ...
    public static String toString(EntityAutocomplete autocompleter) {
        StringBuilder sb = new StringBuilder("[");
        sb.append(autocompleter.getName()).append("{").append(autocompleter.getId()).append("}@").append(autocompleter.getUrl());
        return sb.toString();
    }
