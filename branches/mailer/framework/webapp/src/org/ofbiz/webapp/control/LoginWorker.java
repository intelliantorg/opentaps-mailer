    ...
    public static String checkLogin(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (userLogin == null) {
            ...
            if (username == null) {
            	...
            	// check if this is a single sign-on user
            	// if so, retrieve the password with username
            	if (request.getRemoteUser() != null) {  // SSO user request
            		username = request.getRemoteUser();

            		Debug.log("Found single sign-on username [" + username + "] in the request.", module);
            		...
            		if (delegator != null) {
            			try {
            				...
            				if (userLogin != null) {  // SSO user is found in the database
            					password = userLogin.getString("currentPassword");

            					if (password != null) {
            						Debug.log("The single sign-on user [" + username + "] has a password.", module);
            					}
            				}
            				else {  // SSO user is not found in the database
            					username = null;
            					Debug.logWarning("The single sign-on user [" + username + "] has no UserLogin entry.", module);
            				}
            			}
            			catch (GenericEntityException e) {
            		...
            		if (username != null && password == null) {
            			Debug.logWarning("The single sign-on user [" + username + "] has null password.", module);
            			return "error";
            		}
            	}
            }
