    ...
    public Iterator findUserLoginSecurityGroupByUserLoginId(String userLoginId) {
        ...
        if (collection == null) {
            try {
                ...
                if (collection != null) {
                    // only store in cache if we get something
                    userLoginSecurityGroupByUserLoginId.put(userLoginId, collection);
                } else {
                    Debug.logInfo("User login ["+userLoginId+"] has no UserLoginSecurityGroup entries after databse lookup.", module);
                    collection = new LinkedList();
                }
            } catch (GenericEntityException e) {
                Debug.logWarning("Cannot lookup UserLoginSecurityGroup due to database issue.  User login ["+userLoginId+"] will fail security for the time being: " + e.getMessage(), module);
                collection = new LinkedList();
            }
        }
