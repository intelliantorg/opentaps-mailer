...
public class UtilDateTime {
     ...
     */
    private static final String DATE_TIME_FORMAT = "MM/dd/yy HH:mm";
    ...
    public static Timestamp addDaysToTimestamp(Timestamp start, Double days) {
        return new Timestamp(start.getTime() + ((int) (24*60*60*1000*days.intValue())));
    }
     ...
     */
    public static List getMonthNames(Locale locale) {
        Calendar tempCal = Calendar.getInstance(locale);
    ...
    public static Timestamp stringToTimeStamp(String dateTimeString, String dateTimeFormat, TimeZone tz, Locale locale) throws ParseException {
        ...
        if (!UtilValidate.isDateTime(dateTimeString, dateTimeFormat, locale, tz))
            throw new ParseException("Invalid format. Could not convert " + dateTimeString + " to Timestamp with pattern " + dateTimeFormat + ".", 0);
