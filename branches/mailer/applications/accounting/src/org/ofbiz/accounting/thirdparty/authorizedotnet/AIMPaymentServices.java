...
/**
 * Important notes about test mode:  There are two different setups for testing.  The first one uses the live
 * Authorize.NET server with the credentials of a real account.  The second one uses a special test Authorize.NET
 * server with the credentials of a separate test account.  These two accounts, the live and the testing, cannot be
 * the same.  Furthermore, both servers behave exactly the same with one major difference:  The test server will
 * not send information to financial institutions, thus any transactions performed on the test server are not real.
 *
 * Both servers can be set to TEST mode, which will disable transaction IDs from being generated.  What this means is
 * with TEST mode, you will not be able to perform authorizations followed by a capture at a later point, refunds, or
 * anything that requires a prior transaction ID to work.  Hence, when you are using the test server and want to
 * perform such transactions, make sure the TEST mode is off.  If this is unclear, just ignore TEST mode altogether, it
 * is an artifact of an older Authorize.NET credentials policy. 
 */
public class AIMPaymentServices {
     ...
     * time limit
     */
    public static final List VOIDABLE_RESPONSES_NO_TIME_LIMIT = UtilMisc.toList("50");
     ...
     */
    public static final List VOIDABLE_RESPONSES_TIME_LIMIT = UtilMisc.toList("54");

    /**
     * The number of days in the time limit when one can safely consider an unsettled
     * transaction to be still valid
     */
    public static int TIME_LIMIT_VERIFICATION_DAYS = 120;
    ...
    /**
     * Authorize.NET provides a Transaction Cut-Off Time, which is the time when all pending transactions for the day
     * are processed.  This results in a complication for refunds, because an Authorize.NET CREDIT can only be issued
     * on CAPTUREs that were processed and settled after this time.
     *
     * Hence, the strategy of this service is to attempt to issue a CREDIT.  If it fails, then it will VOID the full
     * amount and then let the refundPayment service reauthorize the balance. 
     */
    public static Map ccRefund(DispatchContext ctx, Map context) {
        ...
        // if the CREDIT fails, then VOID the full amount but only if the transaction is voidable
        if (!refundResult && isVoidable(refundFlag, authTransaction)) {
            Double authAmountObj = authTransaction.getDouble("amount");
            Double refundAmountObj = (Double)context.get("refundAmount");

            BigDecimal authAmount = authAmountObj != null? new BigDecimal(authAmountObj.toString()) : BigDecimal.ZERO;
            BigDecimal refundAmount = refundAmountObj != null? new BigDecimal(refundAmountObj.doubleValue()) : BigDecimal.ZERO;

            authAmount = authAmount.setScale(2, BigDecimal.ROUND_HALF_EVEN);
            ...
            // since we're voiding the entire original transaction, we should tell the upstream services that the refund
            // is actually the original amount
            results.put("refundAmount", new Double(authAmount.doubleValue()));
        }

        return results;
    }

    // checks if the given authorization transaction is voidable based on the response code from a failed CREDIT transaction
    private static boolean isVoidable(String responseCode, GenericValue authTransaction) {
        if (VOIDABLE_RESPONSES_NO_TIME_LIMIT.contains(responseCode)) return true;
        if (VOIDABLE_RESPONSES_TIME_LIMIT.contains(responseCode)) {
            ...
            return startTimestamp.before(authTimestamp);
        }
        return false;
    }
    ...
    private static HashMap processCard(Map request, Properties props) {
        ...
        if (url == null || url.length() == 0) {
            ...
            Debug.logWarning("No payment.authorizedotnet.url found.  Using certification URL for testing: [" + url + "]", module);
        }
        if(isLoggingSensitive()) {
            Debug.logInfo("Authorize.net using url [" + url + "]", module);
            Debug.logInfo("Authorize.net request string " + request.toString(),module);
            Debug.logInfo("Authorize.net properties string " + props.toString(),module);
        }
    ...
    // if true, then we send the x_test_request=TRUE flag to the server
    private static boolean isTestMode() {
        return "TRUE".equals( AIMProperties.get("testReq") );
    }

    // if true, then we log sensitive transaction info
    private static boolean isLoggingSensitive() {
        return "TRUE".equals( AIMProperties.get("logging") );
    }
    ...
    private static Properties buildAIMProperties(Map context) {
        ...
        String logging = UtilProperties.getPropertyValue(configStr, "payment.authorizedotnet.logging");
    ...
    private static void buildMerchantInfo(Map params, Properties props, Map AIMRequest) {
        ...
        if (isTestMode()) AIMRequest.put("x_test_request", "TRUE");
    }
    ...
    private static void buildAuthTransaction(Map params, Properties props, Map AIMRequest) {
        ...
        String cardCode = getCardCode(params);
        ...
        if (cardCode != null) AIMRequest.put("x_card_code",cardCode);
    ...
    private static String getCardCode(Map params) {
        String cardCode = null;
        if (params.get("orderPaymentPreference") != null) {
            GenericValue opp = (GenericValue) params.get("orderPaymentPreference");
            if ("CREDIT_CARD".equals(opp.getString("paymentMethodTypeId"))) {
                cardCode = opp.getString("securityCode");
                if (cardCode != null) cardCode = cardCode.trim();
            }
        }
        return cardCode;
    }
