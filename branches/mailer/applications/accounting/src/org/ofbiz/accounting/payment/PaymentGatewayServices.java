    ...
    public static Map refundPayment(DispatchContext dctx, Map context) {
        ...
        if (paymentSettings != null) {
            ...
            if (serviceName != null) {
                ...
                // The actual refund amount can be different from what we request due to issues such
                // as being unable to issue a partial refund
                Double actualRefundAmount = (Double) refundResponse.get("refundAmount");
                if (actualRefundAmount == null) actualRefundAmount = processAmount;
                ...
                try {
                    ...
                    refundResCtx.put("refundAmount", actualRefundAmount);
                ...
                // if we end up refunding more than requested, create a new unauthorized payment pref for the balance
                double balance = actualRefundAmount.doubleValue() - processAmount.doubleValue();
                if (balance > 0.0) {
                    GenericDelegator delegator = dctx.getDelegator();
                    GenericValue newPref = delegator.makeValue("OrderPaymentPreference", paymentPref);
                    newPref.set("statusId", "PAYMENT_NOT_AUTH");
                    newPref.set("maxAmount",  new Double(balance));
                    if (userLogin != null) newPref.put("createdByUserLogin", userLogin.get("userLoginId"));
                    String newPaymentPrefId = null;
                    try {
                        newPaymentPrefId = delegator.getNextSeqId("OrderPaymentPreference");
                        newPref.set("orderPaymentPreferenceId", newPaymentPrefId);
                        newPref.create();
                    } catch (GenericEntityException e) {
                        String msg = "Error while trying to record the new balance of order ["+paymentPref.get("orderId")+"] after refunding it: " + e.getMessage();
                        Debug.logError(e, msg, module);
                        return ServiceUtil.returnError(msg);
                    }

                    // if we're configured to authorize this new payment, then process it
                    String authorizeAgain = UtilProperties.getPropertyValue(paymentConfig, "payment.general.refund.reauth", "false").toLowerCase();
                    if (authorizeAgain.startsWith("t")) {
                        Map input = UtilMisc.toMap("userLogin", context.get("userLogin"));
                        input.put("orderPaymentPreferenceId", newPaymentPrefId);
                        input.put("userLogin", context.get("userLogin"));
                        try {
                            Map authResponse = dispatcher.runSync("authOrderPaymentPreference", input);
                            if (ServiceUtil.isError(authResponse)) return authResponse;
                        } catch (GenericServiceException e) {
                            String msg = "Error while trying to authorize the new balance of order ["+paymentPref.get("orderId")+"] after refunding it: " + e.getMessage();
                            Debug.logError(e, msg, module);
                            return ServiceUtil.returnError(msg);
                        }
                    }
