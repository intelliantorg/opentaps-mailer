    ...
    public static Map createOrder(DispatchContext ctx, Map context) {
        ...
        while (itemIter.hasNext()) {
            ...
            if (orderItemSeqId == null) {
                return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorCannotGetOrderItemSeqId",locale));
            }
            itemValuesBySeqId.put(orderItemSeqId, orderItem);
...
//                            " and for quantity: " + techDataCalendarExcDay.getDouble("usedCapacity").toString(), module);
    ...
    public static Map cancelOrderItem(DispatchContext ctx, Map context) {
        ...
        Debug.logInfo("Found OrderItemShipGroupAssoc to cancel : "+orderItemShipGroupAssocs, module);
            ...
            while (i.hasNext()) {
                ...
                if (availableQuantity.doubleValue() >= thisCancelQty.doubleValue()) {
                    if (availableQuantity.doubleValue() == 0) {
                        Debug.logInfo("OrderItemShipGroupAssoc ["+orderId+"/"+orderItemShipGroupAssoc.get("orderItemSeqId")+"/"+orderItemShipGroupAssoc.get("shipGroupSeqId")+"] already cancelled", module);
                        continue;
                    }
                    Debug.logInfo("OrderItemShipGroupAssoc ["+orderId+"/"+orderItemShipGroupAssoc.get("orderItemSeqId")+"/"+orderItemShipGroupAssoc.get("shipGroupSeqId")+"] has "+thisCancelQty+" to cancel ...", module);
                    Double orderItemQtyCancelled = itemCancelQuantity + thisCancelQty;
                    orderItem.set("cancelQuantity", orderItemQtyCancelled);
                    Debug.logInfo("OrderItem qty cancelled now  = " + orderItem.get("cancelQuantity"), module);
                    ...
                    if (orderItemQtyCancelled.doubleValue() >= itemQuantity.doubleValue()) {
    ...
    public static Map updateApprovedOrderItems(DispatchContext dctx, Map context) {
        ...
        while (iai.hasNext()) {
            ...
            if (cartItem != null) {
                ...
                // get cancelled qty
                try {
                    GenericValue oi = delegator.findByPrimaryKey("OrderItem", UtilMisc.toMap("orderId", orderId, "orderItemSeqId",itemSeqId ));
                    if (oi != null) {
                        Double cancelQuantity = (Double)oi.get("cancelQuantity");
                        if (cancelQuantity != null) {
                            qty += cancelQuantity;
                            Debug.logWarning("Got cancelled qty for item : "+itemSeqId+" = "+cancelQuantity, module);
                        }
                    }
                } catch (GenericEntityException e) {
                    Debug.logError(e, module);
                ...
                Debug.logWarning("Set item quantity: [" + itemSeqId + "] " + qty, module);
        ...
        while (gai.hasNext()) {
            ...
            if (cartItem != null) {
                Debug.logWarning("Shipping info (before) for group #" + (groupIdx-1) + " [" + cart.getShipmentMethodTypeId(groupIdx-1) + " / " + cart.getCarrierPartyId(groupIdx-1) + "]", module);
                Debug.logWarning("Set new group qty = "+groupQty, module);
        ...
        // save all the updated information
        try {
            Debug.logWarning("Save cart ...", module);
            saveUpdatedCartToOrder(dispatcher, delegator, cart, locale, userLogin, orderId);
        } catch (GeneralException e) {
            return ServiceUtil.returnError(e.getMessage());
        }
        ...
        // log an order note
        try {
            Debug.logWarning("Create order note ...", module);
